...
export class CommercialContactsComponent implements OnInit {
  ...
  getRowData(startRow: number, endRow: number, sort, filter): Observable<any[]> {
    ...
    if (sort.length) {
      ...
      if (sortField == 'mandate') {
        sortData = 'data.commercial1.data.address.formatted_address'
    ...
    if (this.tabType == 'myListing') {
      searchData = searchData + '&data.commercial1.data.user.data.email=' + this.email
      let url = 'https://whitefang-digitaloffice.form.io/commercialcontact/submission?sort=' + sortData + '&skip=' + startRow + '&limit=' + limit + '&select=' + selectFields + '&' + searchData;
  ...
  getFieldName(name){
    if (name == 'mandate') {
      return 'data.commercial1.data.address.formatted_address'
        ...
        this.getRowData(params.startRow, params.endRow, params.sortModel, params.filterModel)
          .subscribe(data => {
            ...
            for (let i = 0; i < data.length; i++) {
              let element = data[i];
              parSedData.push({
                "agentName":`${element.data.commercial1.data.user.data.firstName+' '+element.data.commercial1.data.user.data.lastName}`,
                "mandate": element.data.commercial1.data ? element.data.commercial1.data.address.formatted_address : '-',
  ...
  getTotalRows() {
    ...
    return new Promise(resolve => {
      let headers = new HttpHeaders().set('x-token', 'C7rBtDpCVAXqjx4RPOjD2jpe0Xati6').set('content-type', 'application/json');
      this.http.get<any[]>('https://whitefang-digitaloffice.form.io/commercialcontact/submission?skip=0&limit=100000&select=_id,data.commercial1.data.user.data.office._id'+'&'+ searchData, { headers }).subscribe(resp => {
        resolve(resp.length)
      })
    })
  }
